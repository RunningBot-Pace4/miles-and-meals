...
vi.mock("@/lib/google-places", async (original) => {
  const actual = await original<typeof import("@/lib/google-places")>();
  ...
  return { ...actual, searchGooglePlace: resolve, getGooglePlace: resolve };
});
...
beforeEach(() => { Object.assign(state, { loggedIn: true, access: true, country: true, calls: [] }); process.env.GOOGLE_MAPS_PLATFORM_API_KEY = "secret"; });
...
describe("Google location resolver API", () => {
  ...
  it("refreshes a permitted stored Place ID without searching by title", async () => {
