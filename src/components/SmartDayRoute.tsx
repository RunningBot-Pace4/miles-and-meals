...
export function SmartDayRoute({ items, countryId, tripName, disabled, onUpdated, onPin, initialDate }: {
  initialDate?: string; items: SmartRouteItem[]; countryId: string; tripName: string; disabled: boolean; onUpdated: () => Promise<void>; onPin: (id: string) => void;
}) {
  ...
  return <section className="smart-day-route" aria-labelledby="smart-route-title">
    ...
    {!dates.length ? <p className="muted">Add dates to itinerary activities to build a day route.</p> : <>
      ...
      <div className="smart-route-actions"><button className="button secondary" disabled={busy || disabled || analysis.ordered.length < 2} type="button" onClick={() => void applyOrder()}>{busy ? "Applying…" : "Apply suggested order"}</button>{routeUrl ? <a className="button secondary" href={routeUrl} target="_blank" rel="noreferrer">Open in Google Maps ↗</a> : null}</div>
