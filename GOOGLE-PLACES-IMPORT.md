...
## Takeout and exact pin corrections
Unmatched Takeout rows can now be imported without a location. A failed lookup does not invalidate the CSV. Each import row and saved place has Set exact pin / Set or correct pin. The stay editor also has Choose pin on map. Zoom and tap the exact building, then confirm. Coordinates and full pin links can be pasted if the map cannot load. Exact imported or selected pins take priority over name search. The map view alone never sets a pin. Stored pins allow distance calculation without repeated geocoding. Distances are straight-line km, not walking or driving routes. Correct an existing incorrectly saved stay before trusting its distances. Attribution moved from repeated controls to the footer; map attribution remains visible. No paid Google lookup is used.
