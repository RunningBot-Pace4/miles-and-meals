...
must(packageJson, '"version": "1.92.2"', "V92.2 package version missing");
...
must(worker, 'miles-meals-static-v92-2', "V92.2 service-worker cache missing");
...
for (const marker of ["NextLink", "useNativeNavigation", 'href.startsWith("/api/")']) {
  must(links, marker, `V92.2 fast internal navigation missing: ${marker}`);
}
must(navigationGate, 'relativePath === "src/components/FullPageLink.tsx"', "Fast navigation exception is not restricted to the central wrapper");
